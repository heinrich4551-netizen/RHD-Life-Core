...
private _effective = (_count max 1) min _playerCap;
...
if (_cfg getOrDefault ["initialPopulationFill",true] && {_cfg getOrDefault ["civilianPopulationEnabled",true]}) then {
    private _existing = count (missionNamespace getVariable ["RHD_LifeCore_Civilians",[]]);
    if (_existing < _target) then {[(_target - _existing) min ((_cfg getOrDefault ["civilianSpawnBatch",12]) max 1)] call RHD_fnc_spawnCivilianBatch;};
};
...
while {missionNamespace getVariable ["RHD_LifeCore_WorldStarted",false]} do {
    ...
    _effective = ((_count max 1) min ((_cfg getOrDefault ["civilianPlayerCap",12]) max 1));
