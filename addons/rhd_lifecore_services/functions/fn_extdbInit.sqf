...
private _adapter=toLower (_cfg getOrDefault ['storageAdapter','profilenamespace']);
...
private _protoRes='extDB3' callExtension format ['9:ADD_DATABASE_PROTOCOL:%1:%2:SQL_RAW',_db,_protocol];
private _id=parseNumber _protoRes;
if (_id <= 0) exitWith {diag_log format ['[RHD LifeCore] EXTDB3 unavailable or protocol init failed: DB=%1 result=%2 protocol=%3',_db,_dbRes,_protoRes];false};
