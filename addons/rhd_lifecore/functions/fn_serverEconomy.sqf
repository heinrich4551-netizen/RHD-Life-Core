...
switch (toLower _action) do {
    ...
    case "deposit": {
        ...
        if (_amount <= _cash) then {
            ...
            _profile set ["bank", (_profile getOrDefault ["bank", 0]) + _amount];
            _changed = true;
        };
    };
    case "withdraw": {
        ...
        if (_amount <= _bank) then {
            ...
            _profile set ["cash", (_profile getOrDefault ["cash", 0]) + _amount];
            _changed = true;
        };
    };
    case "transfer": {
        ...
        private _bank = _profile getOrDefault ["bank", 0];
        private _target = _profiles getOrDefault [_targetUID, createHashMap];
        if (count _target > 0 && {_amount > 0} && {_amount <= _bank}) then {
            _profile set ["bank", _bank - _amount];
